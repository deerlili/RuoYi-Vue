...
public class DimReviewInfoController extends BaseController
{
     ...
     * 查询追溯号管理列表
     */
    @PreAuthorize("@ss.hasPermi('knife:info:list')")
    @GetMapping("/list")
     ...
     * 导出追溯号管理列表
     */
    @PreAuthorize("@ss.hasPermi('knife:info:export')")
     ...
     * 获取追溯号管理详细信息
     */
    @PreAuthorize("@ss.hasPermi('knife:info:query')")
     ...
     * 新增追溯号管理
     */
    @PreAuthorize("@ss.hasPermi('knife:info:add')")
    @Log(title = "追溯号管理", businessType = BusinessType.INSERT)
    @PostMapping
    public AjaxResult add(@RequestBody DimReviewInfo dimReviewInfo)
    {
        for (int i = 0; i < Integer.parseInt(dimReviewInfo.getNum()); i++) {
            dimReviewInfoService.insertDimReviewInfo(dimReviewInfo);
        }
        return toAjax(Integer.parseInt(dimReviewInfo.getNum()));
    }

    /**
     * 修改追溯号管理
     */
    @PreAuthorize("@ss.hasPermi('knife:info:edit')")
     ...
     * 删除追溯号管理
     */
    @PreAuthorize("@ss.hasPermi('knife:info:remove')")
